...
CodeMirror.defineMode("perl6",function(config,parserConfig){
	...
	for(i = 0; i < p6Type.length; i++) {
        words = p6Type[i].split(' ');
        for(j = 0; j < words.length; j++) {
            PERL[words[j]] = 1;
        }
    }

	var p6Keyword = p6DeclareRoutine
         .concat(p6Module)
         .concat(p6Variable)
         .concat(p6Include)
         .concat(p6Conditional)
         .concat(p6VarStorage)
         .concat(p6Repeat)
         .concat(p6FlowControl)
         .concat(p6TypeConstraint)
         .concat(p6ClosureTrait)
         .concat(p6Exception)
         .concat(p6Property)
         .concat(p6Number)
         .concat(p6Pragma);
	for(i = 0; i < p6Keyword.length; i++) {
        words = p6Keyword[i].split(' ');
        for(j = 0; j < words.length; j++) {
            PERL[words[j]] = 1;
         }
    }
