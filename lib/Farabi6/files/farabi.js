...
function startFarabi(editorId) {
	...
	editor.on("change", function(cm) {
		if( !$("#sync_pod_button").hasClass('active') ) {
			return;
		}
	...
	var syntaxCheck = function() {
		...
		$.post('/syntax_check', {"source": editor.getValue() }, function(problems) {
			...
			for (i = 0; i < syntaxCheckWidgets.length; i++) {
				editor.removeLineWidget(syntaxCheckWidgets[i].widget);
			}
			...
			if(problems.length > 0) {
				for(i = 0; i < problems.length; i++) {
					...
					syntaxCheckWidgets.push({
						'problem' : problem,
						'widget'  : editor.addLineWidget(error.line_number - 1, msg, {coverGutter: true, noHScroll: true}),
						'node'    : msg
					});
				}
	...
	$("#sync_pod_button").click(function() {
		if ($(this).has('active')) {
			podChanged(editor);
		}
	});
