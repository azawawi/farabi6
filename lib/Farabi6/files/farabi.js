...
function startFarabi(editorId) {
	...
	$("#ok-button", $runDialog).click(function() {
		...
		if(runtime == "perlito-6") {
			...
			// Rakudo Perl 6
			$.get('/run/rakudo', {"source": editor.getValue() }, function(result) {
				$output.val(result.output);
			});
		} else if(runtime == "niecza") {
			// Niecza
			$.get('/run/niecza', {"source": editor.getValue() }, function(result) {
				$output.val(result.output);
			});
		}
	});
	...
	$(".new-file-button").click(function() {
		$("#myTab").append('<li class="active"><a href="#home" data-toggle="tab">Home&nbsp;&nbsp;<i class="icon-remove"></i></a></li>');
	});
}
