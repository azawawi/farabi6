...
function startFarabi(editorId) {
	...
	var $syncPodButton = $("#sync_pod_button");
	editor.on("change", function(cm) {
		clearTimeout(self.timeoutId);
		self.timeoutId = setTimeout(function() {
			if ( $syncPodButton.hasClass('active') ) {
				podChanged(cm);
			}
			if ( $syntaxCheckButton.hasClass('active') ) {
				syntaxCheck(cm);
			}
		}, 500);
	});
	...
	setTimeout(function() {
		...
		// Syntax check editor contents at startup
		syntaxCheck(editor);
	...
	$(".open_url_button").click(function() {
		var url = "https://raw.github.com/ihrd/uri/master/lib/URI.pm";
		$.post('/open_url',
                        { "url": url },
                        function(code) {
	...
	var syntaxCheck = function(cm) {
		...
		$.post('/syntax_check', {"source": cm.getValue() }, function(problems) {
			var i, problem;
			...
			if(problems.length > 0) {
				for(i = 0; i < problems.length; i++) {
					...
					syntaxCheckWidgets.push({
						'problem' : problem,
						'widget'  : cm.addLineWidget(error.line_number - 1, msg, {coverGutter: true, noHScroll: true}),
						'node'    : msg
					});
				}
